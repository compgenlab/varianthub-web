...
func (t Token) Active() bool { return t.RevokedAt == 0 }

// CreateToken mints a personal API token, returning the secret to show once.
func (s *Store) CreateToken(ctx context.Context, userID, name string) (Token, string, error) {
	secret, prefix, hash, err := NewToken()
	if err != nil {
		return Token{}, "", err
	}
	t := Token{ID: NewID(), Name: strings.TrimSpace(name), Prefix: prefix, CreatedAt: s.nowFn()}
	_, err = s.pool.Exec(ctx, `
		INSERT INTO api_token (id,user_id,name,prefix,hash,created_at)
		VALUES ($1,$2,$3,$4,$5,$6)`,
		t.ID, userID, t.Name, t.Prefix, hash, t.CreatedAt)
	if err != nil {
		return Token{}, "", err
	}
	return t, secret, nil
